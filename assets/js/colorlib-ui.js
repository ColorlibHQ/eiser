/*! ColorlibUI 3.0.0 — core, owl, magnific, ajaxchimp, parallax, owl markup 2.2. Built for this theme from core + the modules it uses. */
/**
...
/* ColorlibUI module: parallax — replaces the `$.fn.parallax` the themes get from
 * the second plugin bundled at the end of their stellar.js (odahcam's
 * jquery.parallax; Stellar itself is never started). That plugin, on every
 * scroll, moved each in-view element by translateY((scrollTop − offset().top) / 2);
 * because offset() already includes the element's own transform, the value
 * feeds back on itself and settles at a third of the distance, jittering on
 * large jumps. This computes that settled value directly, from the untransformed
 * position, once per animation frame: translateY((scrollY − top) × speed) with
 * speed 1/3 — the same movement without the jitter. data-background (when not
 * empty) becomes the background image. Nothing moves with reduced motion.
 * { stellar: true } (and ColorlibUI.stellar()) instead follow the markup's
 * Stellar.js attributes: data-stellar-ratio / data-stellar-vertical-offset move
 * elements by (scrollY + offset − parent top) × (1 − ratio), and
 * data-stellar-background-ratio shifts background-position-y the same way. */
(function () {
  'use strict';
  var UI = window.ColorlibUI;
  if (!UI) return;
  ...
  /** Collect what moves under `root` and how, measured without our own transforms. */
  function Item(el, mode, o) {
    this.el = el;
    this.mode = mode;
    this.shift = 0;
    this.offset = num(el.getAttribute('data-stellar-vertical-offset'), o.verticalOffset || 0);
    if (mode === 'bg') {
      this.ratio = num(el.getAttribute('data-stellar-background-ratio'), 1);
      this.fixed = getComputedStyle(el).backgroundAttachment === 'fixed';
      this.bgStart = num(getComputedStyle(el).backgroundPositionY, 0);
    } else if (mode === 'stellar') {
      this.ratio = num(el.getAttribute('data-stellar-ratio'), 1);
    }
    this.measure();
  }
  Item.prototype.measure = function () {
    var el = this.el;
    // The layout position: our translate is subtracted back out.
    this.top = docTop(el) - (this.mode === 'bg' ? 0 : this.shift);
    this.height = el.offsetHeight;
    var p = el.offsetParent;
    this.parentTop = p ? docTop(p) + p.clientTop : 0;
  };
  Item.prototype.update = function (y, vh, speed) {
    var el = this.el;
    if (this.mode === 'bg') {
      var k = (this.fixed ? 0 : 1) - this.ratio;
      el.style.backgroundPositionY = (y + this.offset - this.top - this.bgStart) * k + 'px';
      return;
    }
    if (this.mode === 'stellar') this.shift = (y + this.offset - this.parentTop) * (1 - this.ratio);
    else {
      // Like the plugin, only elements whose (moved) box is on screen are updated.
      var top = this.top + this.shift;
      if (y > top + this.height || y + vh < top) return;
      this.shift = (y - this.top) * speed;
    }
    el.style.transform = 'translateY(' + this.shift + 'px)';
  };

  function setBackground(el) {
    var src = el.getAttribute('data-background');
    if (src && !el.style.backgroundImage) el.style.backgroundImage = 'url("' + src.replace(/"/g, '\\"') + '")';
  }

  /** $('.bg-parallax').parallax() → ColorlibUI.parallax('.bg-parallax'). */
  UI.parallax = function (target, options) {
    var o = UI.extend({ speed: 1 / 3, stellar: false }, options);
    var out = [];
    UI.ready(function () {
      var items = [];
      UI.toElements(target).forEach(function (root) {
        setBackground(root);
        Array.prototype.forEach.call(root.querySelectorAll('[data-background]'), setBackground);
        if (UI.reducedMotion) return;
        if (!o.stellar) {
          if (root._clParallax) return;
          root._clParallax = true;
          root.setAttribute('data-cl-parallax', '1');
          items.push(new Item(root, 'plain', o));
          return;
        }
        var list = [root].concat(Array.prototype.slice.call(root.querySelectorAll('[data-stellar-ratio], [data-stellar-background-ratio]')));
        list.forEach(function (el) {
          if (el._clParallax) return;
          if (el.hasAttribute('data-stellar-ratio')) { el._clParallax = true; items.push(new Item(el, 'stellar', o)); }
          if (el.hasAttribute('data-stellar-background-ratio')) { el._clParallax = true; items.push(new Item(el, 'bg', o)); }
        });
      });
      ...
      function frame() {
        queued = false;
        var y = window.pageYOffset, vh = document.documentElement.clientHeight;
        items.forEach(function (it) { it.update(y, vh, o.speed); });
      }
      function request() {
      ...
      window.addEventListener('resize', UI.debounce(function () {
        items.forEach(function (it) { it.measure(); });
        request();
      }, 100));
      request();
      out.push({ items: items, refresh: function () { items.forEach(function (it) { it.measure(); }); request(); } });
    });
    return out;
  };

  /** $(window).stellar(options) → ColorlibUI.stellar(options): every Stellar element on the page. */
  UI.stellar = function (options) {
    return UI.parallax(document.body, UI.extend({}, options, { stellar: true }));
  };
}());
